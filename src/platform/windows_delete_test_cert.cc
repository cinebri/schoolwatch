// https://github.com/schoolwatch/schoolwatch/discussions/6444#discussioncomment-9010062
...
//*************************************************************
//
//  DeleteSchoolWatchTestCertsW_SingleHive()
//
//  Purpose:    Deletes SchoolWatch Test certificates and wrong key stores
//
...
BOOL DeleteSchoolWatchTestCertsW_SingleHive(HKEY RootKey, LPWSTR Prefix = NULL) {
...
extern "C" void DeleteSchoolWatchTestCertsW() {
	// Current user
	std::wcout << "*** Current User" << std::endl;
	DeleteSchoolWatchTestCertsW_SingleHive(HKEY_CURRENT_USER);

	// Local machine (requires admin rights)
	std::wcout << "*** Local Machine" << std::endl;
	DeleteSchoolWatchTestCertsW_SingleHive(HKEY_LOCAL_MACHINE);
	...
	for (DWORD Index = 0; ; Index++) {
		...
		DeleteSchoolWatchTestCertsW_SingleHive(HKEY_USERS, SubKeyName);
	}
