...
fn get_plugin_source_list() -> Vec<PluginSource> {
    ...
    // vec![PluginSource {
    //     name: "schoolwatch".to_string(),
    //     url: "https://raw.githubusercontent.com/fufesou/schoolwatch-plugins/main".to_string(),
...
fn elevate_install(
    ...
    crate::platform::elevate(args, "SchoolWatch wants to install then plugin")
}
...
fn elevate_uninstall(plugin_id: &str) -> ResultType<bool> {
    crate::platform::elevate(
        ...
        "SchoolWatch wants to uninstall the plugin",
    )
}
