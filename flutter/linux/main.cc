...
#define RUSTDESK_LIB_PATH "libschoolwatch.so"
typedef bool (*SchoolWatchCoreMain)();
bool gIsConnectionManager = false;

void print_help_install_pkg(const char* so);

bool flutter_schoolwatch_core_main() {
   void* libschoolwatch = dlopen(RUSTDESK_LIB_PATH, RTLD_LAZY);
   if (!libschoolwatch) {
      fprintf(stderr,"Failed to load \"libschoolwatch.so\"\n");
      char* error;
   ...
   auto core_main = (SchoolWatchCoreMain) dlsym(libschoolwatch,"schoolwatch_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
       fprintf(stderr, "Program entry \"schoolwatch_core_main\" is not found: %s\n", error);
       return false;
   }
   return core_main();
}
...
int main(int argc, char** argv) {
  if (!flutter_schoolwatch_core_main()) {
      return 0;
  }
