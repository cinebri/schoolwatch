...
void msgBox(SessionID sessionId, String type, String title, String text,
  ...
  submit() {
    dialogManager.dismissAll();
    // https://github.com/schoolwatch/schoolwatch/blob/5e9a31340b899822090a3731769ae79c6bf5f3e5/src/ui/common.tis#L263
      ...
      if (lpos.isMaximized == true) {
        ...
        if (isWindows) {
          if (storeSize) {
            ...
            // See known issue in https://github.com/schoolwatch/schoolwatch/pull/9840
...
List<String>? urlLinkToCmdArgs(Uri uri) {
  ...
  if (uri.authority.isEmpty &&
    ...
    // schoolwatch://<connect-id>
    // schoolwatch://<connect-id>/r
    // schoolwatch://<connect-id>/r@<server>
    command = '--connect';
    id = uri.authority;
    if (uri.path.length > 1) {
      id = id + uri.path;
...
Future<void> onActiveWindowChanged() async {
  ...
  if (rustDeskWinManager.getActiveWindows().isEmpty) {
    ...
    try {
      ...
      debugPrint("Start closing SchoolWatch...");
      ...
      if (isMacOS) {
        ...
        // 2024-11-11 11:41:11.546 SchoolWatch[90272:2567686] Failed to create a FlutterPlatformMessageResponseHandle (2)
        // embedder.cc (2672): 'FlutterEngineSendPlatformMessage' returned 'kInvalidArguments'. Invalid engine handle.
        // 2024-11-11 11:41:11.565 SchoolWatch[90272:2567686] Failed to send message to Flutter engine on channel 'flutter/lifecycle' (2).
        // ```
        periodic_immediate(
...
class ServerConfig {
  ...
  /// decode from shared string (from user shared or schoolwatch-server generated)
  /// also see [encode]
...
const kMacOSPermChannel = MethodChannel("org.schoolwatch.schoolwatch/macos");
...
setNewConnectWindowFrame(int windowId, String peerId, int preSessionCount,
  ...
  if (screenRect == null) {
    ...
    // https://github.com/schoolwatch/schoolwatch/discussions/8825
...
Widget loadPowered(BuildContext context) {
  return MouseRegion(
    ...
    child: GestureDetector(
      onTap: () {
        launchUrl(Uri.parse('https://schoolwatch.com'));
      },
      child: Opacity(
          opacity: 0.5,
          child: Text(
      ...
      children: [
        Align(
          ...
          style: TextStyle(
            color: Colors.red,
            fontSize:
                18, // https://github.com/schoolwatch/schoolwatch-server-pro/issues/261
            fontWeight: FontWeight.bold,
          ),
...
bool whitelistNotEmpty() {
  // https://schoolwatch.com/docs/en/self-host/client-configuration/advanced-settings/#whitelist
...
// https://github.com/schoolwatch/schoolwatch/blob/ce1dac3b8613596b4d8ae981275f9335489eb935/flutter/lib/desktop/widgets/tabbar_widget.dart#L385
// https://github.com/schoolwatch/schoolwatch/blob/ce1dac3b8613596b4d8ae981275f9335489eb935/flutter/lib/desktop/widgets/tabbar_widget.dart#L399
//
// @platforms macos
