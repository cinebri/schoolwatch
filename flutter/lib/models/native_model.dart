...
class PlatformFFI {
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = isAndroid
        ? DynamicLibrary.open('libschoolwatch.so')
        : isLinux
            ? DynamicLibrary.open('libschoolwatch.so')
            : isWindows
                ? DynamicLibrary.open('libschoolwatch.dll')
                :
                ...
                //
                // isMacOS? DynamicLibrary.open("liblibschoolwatch.dylib") :
  ...
  void _startListenEvent(RustdeskImpl schoolwatchImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = schoolwatchImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
